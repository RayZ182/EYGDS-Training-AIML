...
while True:
    ...
    if user_input.lower().startswith("weather"):
        if "of" not in user_input.lower():
            print("Agent: Please use this format: Weather of 'city' ")
            continue
        else:
            try:
                input_parts = user_input.split()
                location = " ".join(input_parts[2:])
                wt = weather(location)
                print("Agent:", wt)
                memory.save_context({"input": user_input}, {"output": wt})
                continue
            except Exception as e:
                print("Agent: Could not get weather properly:", e)
                continue
